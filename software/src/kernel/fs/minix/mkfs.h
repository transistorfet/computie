...
static int minix_mkfs(device_t dev)
{
	...
	inode_table[root_ino].mode = S_IFDIR | S_IRWXU | S_IRWXG | S_IRWXO;
	inode_table[root_ino].uid = SU_UID;
	inode_table[root_ino].gid = 0;
	inode_table[root_ino].size = 0;
	inode_table[root_ino].nlinks = 1;
	inode_table[root_ino].zones[0] = dir_zone;
	release_block(inode_buf, BCF_DIRTY);
	...
	entries[0].inode = root_ino;
	strcpy(entries[0].filename, ".");
	entries[1].inode = root_ino;
