...
static struct vnode *dir_setup(struct vnode *vnode, struct vnode *parent)
{
	...
	zone = minix_alloc_zone(MINIX_SUPER(vnode->mp->super));
	buf = get_block(vnode->mp->dev, zone);
	if (!buf) {
		vfs_release_vnode(vnode);
		return NULL;
	}
	MINIX_DATA(vnode).zones[0] = buf->num;
	...
	// TODO this still isn't right because parent could be a non-minix vnode
	entries[1].inode = parent ? MINIX_DATA(parent).ino : MINIX_DATA(vnode).ino;
